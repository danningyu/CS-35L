...
for (root, dirs, files) in os.walk(branchDirectory, topdown=True): 
        ...
        for file in files:
            ...
            #branchPaths.append(os.path.join(root, file))
...
for branchHash in branchHashes.keys():
    # print("Starting from branch " + branchHash)
    if nodeDict.get(branchHash) is None:
        branchNode = CommitNode(branchHash)
        # decObject = get_decomp_object(branchHash)
        ...
        while node_stack:
            ...
            for parentHash in currNode.parents:
                # print("Examining parent " +parentHash)
                parentNode = None;
                if nodeDict.get(parentHash) is not None:
                    # print("Node already exists for hash " + parentHash)
